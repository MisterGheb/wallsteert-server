...
def cancel_order(id):
    ...
    return Response({"success":"Successfully deleted order"}, status_code=204)
...
def match_orders():
    ...
    #Get all list of all stocks_id with open orders
    all_stock_ids = list(set(order.stocks_id for order in all_open_orders))
    for stock_id in all_stock_ids:
        stock = Stock.find_or_fail(stock_id)
        buy_orders = [order for order in all_open_orders if order.type == "BUY"]
        sell_orders = [order for order in all_open_orders if order.type == "SELL"]
        ...
        while (len(buy_orders) > 0 and ((len(sell_orders) > 0 and 
            ...
            if len(sell_orders) > 0 and buy_order.bid_price >= sell_orders[0].bid_price: # checks the price of buy order to check its > sell price
                ...
                if buy_order.executed_volume == buy_order.bid_volume: #If the whole order is complete then close order
                    close_order(buy_order, trade_price)
                    buy_orders.pop(0) #remove the order from the buy orders list
                if sell_order.executed_volume == sell_order.bid_volume: #If the whole order is complete then close order
                    close_order(sell_order, trade_price)
                    sell_orders.pop(0) #remove the order from the sell orders list
                ...
                stock.update(unallocated=stock.unallocated-trade_amount, price=buy_order.bid_price)#update the number of unallocated stocks, and stock price
                buy_order.update(executed_volume=buy_order.executed_volume + trade_amount)
